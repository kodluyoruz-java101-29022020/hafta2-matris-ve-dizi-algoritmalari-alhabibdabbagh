package odev2Algoritmasi.soru3;
	...
	public void birinciMatrisDoldur() {
		for (int i = 0; i < firstMatris.length; i++) {
			for (int j = 0; j < firstMatris[0].length; j++) {
				firstMatris[i][j] = (int) (Math.random() * 10);// Doldurma icin *10 yaptik 0 -10 arasinda olsun rasgele
																// sayilar
		...
		for (int i = 0; i < firstMatris.length; i++) {// birinci matris icin dongu
			for (int j = 0; j < firstMatris[0].length; j++) {// ikinci matris icin dongu
				for (int k = 0; k < secondMatris[0].length; k++) { // resualt matrisi icin dongu
					resualt[i][k] += firstMatris[i][j] * secondMatris[j][k];// for dongusu kullanmadan resualt dizisi
																			// yazdim sonra sadelestirerek ve
																			// birlestirerek ve for dongusu kullanarak
																			// bu duruma gelebildik
				}
